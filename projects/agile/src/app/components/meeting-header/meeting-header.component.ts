...
export class MeetingHeaderComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit() {
      this.initData();
      this.socketservice.getDsNotification('meeting-start')
                  ...
                  this.dailyScrumGrapgQlService.createDailyScrum(dsInputData)
                      ...
                      .subscribe(result => {
                          if (result) {
                              ...
                              this.dsSendNotification(ds.id, +this.sprintId, 'meeting-start', new Date(ds.startedAt));
                              this.dsStarted = true;
